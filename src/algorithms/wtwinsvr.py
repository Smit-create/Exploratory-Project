# Class of Weighted Twin Support Vector Regression
...
class WeightedTSVR(BaseEstimator, RegressorMixin):
    """Weighted Twin Supoort Vector Regression"""
    ...
    def plane1(self, G, f, m):
        ...
        Z3 = np.dot(G, Z2)
        P_ = (Z3 + Z3.T) / 2
        Px_ = np.hstack((P_, np.zeros((m, m))))
        P1 = matrix(np.vstack((Px_, np.zeros((m, 2 * m)))))
        q1 = matrix(
            np.hstack((((f.T) - (np.dot(f.T, P_))), self.e1 * np.ones((1, m)))).reshape(
                -1, 1
            )
        )
        ...
        constraint1 = matrix(
            np.vstack(
                (
                    np.hstack(
                        ((np.vstack((np.eye(m) * -1, np.eye(m)))), np.zeros((2 * m, m)))
                    ),
                    sub,
                )
            )
        )
        constraint_ineq1 = matrix(
            np.hstack(
                (
                    np.hstack((np.zeros(m), np.ones(m) * (self.c1 * self.sai1))),
                    np.zeros(m),
                )
            )
        )
    ...
    def plane2(self, G, h, m):
        """To find the intercept and the coefficient of plane 2."""
        ...
        P_ = (Z3 + Z3.T) / 2
        Px_ = np.hstack((P_, np.zeros((m, m))))
        P2 = matrix(np.vstack((Px_, np.zeros((m, 2 * m)))))
        q2 = matrix(
            np.hstack((((np.dot(h.T, P_) - h.T)), self.e2 * np.ones((1, m)))).reshape(
                -1, 1
            )
        )
        sub = np.hstack((np.zeros((m, m)), -1 * np.eye(m)))
        ...
        constraint_ineq2 = matrix(
            np.hstack(
                (
                    np.hstack((np.zeros(m), np.ones(m) * (self.c2 * self.sai2))),
                    np.zeros(m),
                )
            )
        )
